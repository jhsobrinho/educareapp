...
interface IndividualLicensesTabProps {
  licenses: License[];
  isLoadingTeams: boolean;
  onAllocateTeam: (license: License) => void;
}

const IndividualLicensesTab: React.FC<IndividualLicensesTabProps> = ({
  licenses,
  isLoadingTeams,
  onAllocateTeam
}) => {
    ...
    <div className="space-y-6">
      {licenses.length === 0 ? (
        <div className="text-center p-8 border rounded-md bg-muted/20">
          <p className="text-muted-foreground">Nenhuma licença individual encontrada.</p>
        </div>
      ) : (
...
export default IndividualLicensesTab;
