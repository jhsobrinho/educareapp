...
SET age = CASE 
  WHEN birthdate IS NOT NULL THEN 
    GREATEST(0, CEIL(EXTRACT(EPOCH FROM (CURRENT_DATE - birthdate::date)) / (30.44 * 24 * 3600)))::INTEGER
  ELSE 0 
END,
updated_at = now()
...
BEGIN
    ...
    NEW.age := CASE 
        WHEN NEW.birthdate IS NOT NULL THEN 
            GREATEST(0, CEIL(EXTRACT(EPOCH FROM (CURRENT_DATE - NEW.birthdate::date)) / (30.44 * 24 * 3600)))::INTEGER
        ELSE 0 
    END;
