 ...
 * 
 * Uso: node import-journey-v2-data-fixed.js
 */
...
// Conjunto para rastrear badges já criadas
const createdBadges = new Set();
...
/**
 * Cria uma badge se ela não existir
 */
async function createBadgeIfNotExists(badgeData, weekId, type) {
  try {
    // Verificar se a badge já foi criada nesta execução
    if (createdBadges.has(badgeData.id)) {
      console.log(`Badge ${badgeData.id} já foi criada nesta execução, pulando...`);
      return;
    }
    
    // Verificar se a badge já existe no banco de dados
    const existingBadge = await JourneyV2Badge.findByPk(badgeData.id);
    
    if (!existingBadge) {
      await JourneyV2Badge.create({
        id: badgeData.id,
        name: badgeData.nome,
        icon: badgeData.icone,
        description: badgeData.description,
        type: type,
        week_id: weekId
      });
      
      // Adicionar ao conjunto de badges criadas
      createdBadges.add(badgeData.id);
      console.log(`Badge ${badgeData.id} criada com sucesso.`);
    } else {
      console.log(`Badge ${badgeData.id} já existe no banco de dados, pulando...`);
    }
  } catch (error) {
    console.error(`Erro ao criar badge ${badgeData.id}:`, error);
  }
}
          ...
          if (weekData.badge_on_complete) {
            if (typeof weekData.badge_on_complete === 'object' && !Array.isArray(weekData.badge_on_complete)) {
              ...
              if (weekData.badge_on_complete.baby) {
                await createBadgeIfNotExists(weekData.badge_on_complete.baby, weekRecord.id, 'baby');
              }
              
              if (weekData.badge_on_complete.mother) {
                await createBadgeIfNotExists(weekData.badge_on_complete.mother, weekRecord.id, 'mother');
              }
              
              // Badge geral
              if (weekData.badge_on_complete.id) {
                await createBadgeIfNotExists(weekData.badge_on_complete, weekRecord.id, 'general');
              }
            }
          }
        } else {
