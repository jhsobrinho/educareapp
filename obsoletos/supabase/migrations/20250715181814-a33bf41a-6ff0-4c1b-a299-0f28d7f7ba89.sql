-- Corrigir função calculate_child_modules para resolver erro de comparação JSON
...
BEGIN
    ...
    -- Pegar primeiro elemento se for array não-vazio
    IF current_module IS NOT NULL AND json_array_length(current_module) > 0 THEN
        current_module := current_module->0;
    ELSE
        current_module := null;
    END IF;
    
    IF next_module IS NOT NULL AND json_array_length(next_module) > 0 THEN
